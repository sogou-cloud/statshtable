...
import com.urbanairship.statshtable.OpType;
...
public class StatsHTableTest extends NeedsEmbeddedCluster {
	...
	private static final byte[] table = StatsHTableTest.class.getName()
			.getBytes();
	...
	public static final void init() throws Exception {
		...
		statsHTablePool = new StatsHTablePool(TEST_UTIL.getConfiguration(),
				true, scope);
	}
	...
	public void afterEach() throws Exception {
		...
		Set<MetricName> metricNames = new HashSet<MetricName>(
				StatsHTable.opTypeTimers.allMetrics().keySet());
	...
	public void basicGetPutTest() throws Exception {
		...
		Assert.assertArrayEquals(key, result.getColumnLatest(cf, key)
				.getValue());
	...
	public void testBatchNewResultArray() throws Exception {
		...
		List<Mutation> puts = new ArrayList<Mutation>();
		Put put = new Put(row);
		put.add(cf, row, row); // put whatever, just write something into the
								// row
		puts.add(put);
		hTable.batchMutate(puts);

		Get get = new Get(row);
		Assert.assertTrue(hTable.exists(get));

		assertMetricsUpdated(OpType.BATCH_MUTATE, OpType.EXISTS);
	}
	...
	public void testBatchPassedResultArray() throws Exception {
		...
		Get get = new Get(row);
		gets.add(get);
		Result[] results = hTable.batchGet(gets);

		Assert.assertArrayEquals(row, (results[0]).getRow());
		assertMetricsUpdated(OpType.BATCH_GET);
	}
	...
	public void testGetScannerScanOnly() throws Exception {
		...
		assertMetricsUpdated(OpType.PUT, OpType.GET_SCANNER,
				OpType.RESULTSCANNER_NEXT);
	...
	public void testResultScannerIterator() throws Exception {
		...
		assertMetricsUpdated(OpType.PUT, OpType.GET_SCANNER,
				OpType.RESULTSCANNER_ITERATOR, OpType.RESULTITERATOR_NEXT);

		rs.close();
	}

	/**
	 * Test that the wrapped ResultScanner.next(int) works, and also
	 * ResultScanner.close()
	 */
	@Test
	public void testResultScannerIteratorMultiRowAndClose() throws Exception {
		...
		assertMetricsUpdated(OpType.BATCH_MUTATE, OpType.GET_SCANNER,
				OpType.RESULTSCANNER_NEXTARRAY, OpType.RESULTSCANNER_CLOSE);
	}
	...
	public void testResultScannerIteratorHasNext() throws Exception {
		...
		assertMetricsUpdated(OpType.PUT, OpType.GET_SCANNER,
				OpType.RESULTSCANNER_ITERATOR, OpType.RESULTSCANNER_CLOSE,
				OpType.RESULTITERATOR_HASNEXT);
	}
	...
	public void testMultiDelete() throws Exception {
		...
		assertMetricsUpdated(OpType.PUT, OpType.RESULTSCANNER_NEXT,
				OpType.MULTIDELETE, OpType.RESULTSCANNER_CLOSE,
				OpType.GET_SCANNER);
	...
	/**
	 * Get a random row key guaranteed not to collide with any other keys being
	 * used in the test.
	 */
	...
	/**
	 * Verify that the metrics for the given OpTypes have been updated, and no
	 * others.
	 */
	private void assertMetricsUpdated(List<OpType> opTypes) {
		...
		for (OpType opType : opTypes) {
			expectedMetrics.add(StatsHTable.newMetricName(scope + "_opTypes",
					opType.toString()));
		}

		Map<MetricName, Metric> updatedMetrics = StatsHTable.opTypeTimers
				.allMetrics();

		// Assert that the expected metrics were present and received at least
		// one update
		for (MetricName metricName : expectedMetrics) {
			...
			Timer timerMetric = (Timer) metric;
			if (timerMetric.count() < 1) {
				Assert.fail("Metric should have received an update but had count of 0 for "
						+ metricName);
			}
		}

		// Assert that no other metrics got updated other than the ones
		// specified
